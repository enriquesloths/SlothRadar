...
const draw = (data, _options) => {
	...
	for (var x = 0; x < 1; x++) {
		var base = data.data[2];
		var nyq = base[0].record.radial.nyquist_velocity;
		nyq = nyq / 100;
		for (var i in data.data[2]) {
			try {
				var velVals = base[i].record.velocity.moment_data;
				var side1 = base[parseInt(i) + 1].record.velocity.moment_data;
				var side2 = base[parseInt(i) - 1].record.velocity.moment_data;
				for (var n in velVals) {
					var diffs = [];
					diffs.push(Math.abs(velVals[n] - side1[n]))
					diffs.push(Math.abs(velVals[n] - side2[n]))
					for (var y in diffs) {
						if (diffs[y] > nyq) {
							//console.log(diff)
							if (velVals[n] < 0) {
								velVals[n] = velVals[n] + (nyq * 2);
							} else {
								velVals[n] = velVals[n] - (nyq * 2);
							}
							break;
	...
	rrlEncoded.forEach((radial) => {
		...
		radial.moment_data.forEach((bin, idx) => {
			...
			if (bin.count) {
				...
				var val = bin.value;
				var inspectorVal = bin;

				arr.push((idx + deadZone) * gateSizeScaling)
				valArr.push(val + adder)
	...
	//}
	function requestDealias(cb) {
		if (options.product == 'EL') {
			var formData = new FormData();
			formData.append("file", document.getElementById('hiddenFileUploader').files[0]);
			...
			xhttp.open("POST", "http://127.0.0.1:3333/server/AtticServer/dealias/index.php", true);
	...
	requestDealias(function (values) {
